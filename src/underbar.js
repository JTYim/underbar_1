(function() {
	...
	_.last = function(array, n) {
		var result = n ? array.slice( Math.max(0, array.length-n) ) : array[array.length-1] ;
		return n===0 ? [] : result ;
	};
	...
	_.reject = function(collection, test) {
		...
		return _.filter(collection, function( item, i, collection ){
			return ( !test(item, i, collection) );
		});
	};
	...
	_.reduce = function(collection, iterator, accumulator) {
		_.each(collection, function(item, i, coll){
			accumulator===undefined ? accumulator=item : accumulator=iterator(accumulator, item,i,coll) ;
		});
		return accumulator;
	};
